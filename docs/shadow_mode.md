...
- `shadow_route_target`: Shadow/GUI route path から E2E 用 `/shadow/route/target_point` を生成。
...
- `/shadow/virtual/path`: 現在の実装で使う Shadow Mode route。`shadow_mode_virtual_control` が `livox_lane_detection` scan または PointCloud2 から生成。
- `/shadow/perception/lane_path`: 画像白線 / 路肩検知 IF。`camera_lane_detection` が専用ONNX segmentation modelから `base_link` の `nav_msgs/Path` として publish。
- `/shadow/route/gui_path`: 将来の GUI route IF。`base_link` の `nav_msgs/Path` を想定。
...
- `amaranthus/src/shadow_mode/shadow_route_target/`
